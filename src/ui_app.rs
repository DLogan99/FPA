...
impl ItemForm {
    fn ui(&mut self, ui: &mut egui::Ui, items: &[ItemRecord]) {
        ...
        ui.text_edit_singleline(&mut self.location)
            .hint_text("Location");
        ui.text_edit_singleline(&mut self.reference)
            .hint_text("Reference URL");
        ui.text_edit_singleline(&mut self.cost).hint_text("Cost");
        ui.text_edit_singleline(&mut self.justification)
            .hint_text("Justification");
        ui.text_edit_singleline(&mut self.date)
            .hint_text("Date (YYYY-MM-DD HH:MM)");

        ui.horizontal(|ui| {
            ui.label("Urgency");
            ui.text_edit_singleline(&mut self.urgency);
            ui.label("Value");
            ui.text_edit_singleline(&mut self.value);
        });
        ui.horizontal(|ui| {
            ui.label("Price vs Similar");
            ui.text_edit_singleline(&mut self.price_comp);
            ui.label("Effect");
    ...
    fn to_record(&self, fmt: &str) -> Result<ItemRecord> {
        let date = if self.date.trim().is_empty() {
            ...
            Local
                .datetime_from_str(self.date.trim(), fmt)
...
impl MoneyForm {
    fn ui(&mut self, ui: &mut egui::Ui, items: &[ItemRecord]) {
        ui.text_edit_singleline(&mut self.entry_type)
            .hint_text("Type (income/expense)");
        ui.text_edit_singleline(&mut self.source_or_destination)
            .hint_text("Source or destination");
        ui.text_edit_singleline(&mut self.amount)
            .hint_text("Amount");
        ui.text_edit_singleline(&mut self.notes).hint_text("Notes");
        ui.text_edit_singleline(&mut self.date)
            .hint_text("Date (YYYY-MM-DD HH:MM)");
