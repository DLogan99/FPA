...
pub fn run(cli: Cli, cfg: &AppConfig) -> Result<()> {
    match cli.command {
        Some(Commands::Items(items_cmd)) => handle_items(items_cmd, cfg),
        Some(Commands::Money(money_cmd)) => handle_money(money_cmd, cfg),
        Some(Commands::Settings) => handle_settings(cfg),
        None => show_help_and_wait(),
    }
}
...
fn handle_settings(cfg: &AppConfig) -> Result<()> {
